...
1. **Install completeness:**
   ...
   - does `~/.hermes/agent-hooks/tasklog-gotcha-hermes.mjs` exist + is a `pre_tool_call` entry for it present in the `hooks:` block of `~/.hermes/config.yaml`
...
Summarize as a short ✅/❌ table + targeted suggestions for what's missing. This skill only reports — change nothing.
